...
from utils.eval_utils import parse_gt_rec
from tensorboard_logging import log_scalar, log_images, log_histogram

os.environ["CUDA_VISIBLE_DEVICES"] = "0"
DATA_PATH = 'C:/Users/fengx/Desktop/yolo/keras-yolo3-master/keras-yolo3-master'
...
class Yolo(object):
    def __init__(self):
        ...
        self.step2_batch_size = 8  # note that more GPU memory is required after unfreezing the body 注意，解冻主体后需要更多的GPU内存
        self.yolo_model, self.yolo_body = self.create_model(yolo_weights_path='logs/006/20200321/trained_weights_final.h5') # logs/006/trained_weights_final.h5
                                                                             # logs/006/20200320/trained_weights_final.h5
        # PIL setting
        ...
        if not os.path.exists(self.tmp_gt_files_path):
            ...
            self.train_data, self.val_data, self.val_images = self.read_txt_file_mAP()
...
#
        ...
        y_true = [Input(shape=(h // {0: 32, 1: 16, 2: 8}[l], w // {0: 32, 1: 16, 2: 8}[l],
                               num_anchors // 3, self.num_classes + 5)) for l in range(3)] ### 高斯更改，原self.num_classes + 5
    ...
    def train(self):
        ...
        # Adversarial ground truths # 真实数据
        ...
        loss_save_dict = OrderedDict() # 有序的信息技术，对数据进行排序
        mAP_save_dict = OrderedDict()
        # Yolo Compile # 定义模型训练的模式设置
        ...
        for step in range(start, end):
            ...
            log_scalar(self.callback, 'training loss', y_loss, step) # 记录结果
    ...
    def test(self, epoch, tab='Yolo loss'):
        ...
        log_scalar(self.callback, tab, total_loss, epoch) # 记录结果
        return total_loss

    def eval(self, step, eval_images_path, ground_truth_path, tag='image', is_save_images=False):
        # Add the class predict temp dict
        class_pred_tmp = {}
        for class_name in self.class_names:
            class_pred_tmp[class_name] = []

        ### 获取gt：提取此类的所有gt对象；即所有真实结果框数量，按类统计
            ...
            for i, out in enumerate(zip(out_bboxes_1, out_bboxes_2, out_bboxes_3)):
                ...
                for o, c in enumerate(out_classes):
                    ...
                    if files_id[i] in self.eval_save_images_id:
                        ...
                        color = self.colors[c]
                        cv2.rectangle(image, (new_left, new_top), (new_right, new_bottom), color, 2) # 绘制矩形
                        ###
                        fs_temp = (new_right - new_left) * (new_bottom - new_top)
                        if fs_temp <= 0:
                            fs_temp = 0
                        ###
                        font_size = math.sqrt(fs_temp) / 50
                        if font_size > 0.8:
                            font_size = 0.8
                        elif font_size < 0.3:
                            font_size = 0.3
        ...
        for class_index, class_name in enumerate(sorted(self.gt_counter_per_class.keys())):
            ...
            nd[class_index] = len(predictions_data)      # number of predict data ### nd = len(predictions_data)
            tp = [0] * int(nd[class_index])                  # true positive  ###
            fp = [0] * int(nd[class_index])                  # false positive ###
            ...
            # rec_total.append(mrec)
            # prec_total.append(mprec)
            ap_total.append(ap)
            # rec_total = tp[-1] / float(npos)
            # prec_total = tp[-1] / float(nd)
            sum_AP += ap
        ...
        return mAP * 100, ap_total, mrec, mprec, nd

        # return mAP * 100,ap_total
    ...
    def eval_mAP(self):
        ...
        start = 0
        step = 1
        end = len(self.val_data)
        print('\n----------- start to eval -------------\n')
        print('class eval:')
        # rec_total, prec_total, ap_total = AverageMeter(), AverageMeter(), AverageMeter()
        # mAP,rec,prec,ap,npos,nd, = self.eval((step - start), self.val_images, self.tmp_gt_files_path, 'mAP')
        gt_dict = parse_gt_rec(args.yolo_val_file, args.img_size)
        mAP, ap, rec,prec,nd= self.eval((step - start), self.val_images, self.tmp_gt_files_path, 'mAP')
        print('final mAP: {:.4f}'.format(mAP))
        npos = np.zeros(self.num_classes)
        rec_np = np.array(rec)
        prec_np = np.array(prec)
        for ii in range(self.num_classes):
            class_recs = {}
            for img_id in gt_dict:
                R = [obj for obj in gt_dict[img_id] if obj[-1] == ii]
                bbox = np.array([x[:4] for x in R])
                det = [False] * len(R)
                npos[ii] += len(R)
                class_recs[img_id] = {'bbox': bbox, 'det': det}
            ...
            # rec_total = rec[ii]
            # prec_total = prec[ii]
            print('Class {}: Recall: {:.4f}, Precision: {:.4f}, AP: {:.4f}'.format(ii, rec_np[ii], prec_np[ii], ap[ii]))
